...
public class BuyServlet extends HttpServlet {
        ...
        if (validateForm(request) == true) {
            try {
                ...
                if (carro != null) {
                    ...
                    //Para hacer la tabla de productos del email
                    StringBuilder tablaEmail = new StringBuilder();
                    Map<Producto, Integer> listado = new HashMap<Producto, Integer>();
                    
                    //Comprueba si se puede hacer o no la compra y la hace
                    boolean terminarCompra = persistencia.updateProductIfAvailable(carro.getArticulos(), request, listado);

                    
                    if (terminarCompra == true) {
                        //Si se puede hacer la compra construye la tabla del email
    ...
    protected boolean enviarMail(HttpServletRequest request, Carrito carro, StringBuilder filas, String name, String dir, String email, String formPago) {
    ...
    protected boolean persistenciaCompra(PersistenceInterface persistencia, String client, Carrito cart, String formPago) {
    ...
    protected boolean validateForm(HttpServletRequest request) {
        if (request.getParameterMap().size() >= 6 && request.getParameter("name") != null && request.getParameter("dir") != null && request.getParameter("email") != null && request.getParameter("buy") != null
                && ((request.getParameter("creditCard.x") != null && request.getParameter("creditCard.y") != null)
                || (request.getParameter("reembolso.x") != null && request.getParameter("reembolso.y") != null)
                || (request.getParameter("transfer.x") != null && request.getParameter("transfer.y") != null))) {
            return true;
